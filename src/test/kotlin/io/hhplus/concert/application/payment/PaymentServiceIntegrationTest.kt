...
class PaymentServiceIntegrationTest : BaseIntegrationTest() {
    ...
    @Test
    fun `임시 예약을 한 유저가 동시에 2번 결제하면 1번은 성공하고 1번은 실패한다 (비관적락 적용)`() {
        // given
        ...
        val futureArray = Array(taskCount) {
            CompletableFuture.runAsync {
                try {
                    paymentService.payWithPessimisticLock(
                        PaymentCommand(
                            userId = userId,
    ...
    @Test
    fun `임시 예약을 한 유저가 동시에 2번 결제하면 1번은 성공하고 1번은 실패한다 (분산락 적용)`() {
        // given
        ...
        val futureArray = Array(taskCount) {
            CompletableFuture.runAsync {
                try {
                    paymentService.payWithDistributedLock(
                        PaymentCommand(
                            userId = userId,
                            reservationId = reservationId
                        )
